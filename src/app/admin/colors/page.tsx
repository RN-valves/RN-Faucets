...
export default function AdminColorsPage() {
  ...
  // Data state
  const [colors, setColors] = useState<AdminColorItem[]>([]);
  ...
  const [editingItem, setEditingItem] = useState<AdminColorItem | null>(null);
  const [colorName, setColorName] = useState("");
  const [colorIcon, setColorIcon] = useState("");
  const [colorHex, setColorHex] = useState("#0077B6");
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) {
      setSelectedIds(colors.map((c) => c._id || c.code));
    } else {
      setSelectedIds([]);
    }
  };
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    setErrorMessage("");
    try {
      const res = await addAdminColor({
        name: colorName.trim(),
        icon: colorIcon.trim() || undefined,
        hexCode: colorHex.trim() || undefined,
      });
      if (res) {
        ...
        setErrorMessage("Failed to add color. Name may already exist.");
      }
    } catch {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingItem?._id || !colorName.trim()) return;

    setIsSubmitting(true);
    setErrorMessage("");
    try {
      const res = await updateAdminColor({
        id: editingItem._id,
        name: colorName.trim(),
        icon: colorIcon.trim() || undefined,
        hexCode: colorHex.trim() || undefined,
  ...
  const handleExport = (format: "csv" | "xls") => {
    setExportOpen(false);
    const exportUrl = `/api/colors/export?format=${format}&search=${encodeURIComponent(search)}`;
  ...
  return (
    <div style={{ minHeight: "100vh", background: isDark ? "#06090E" : "#F4F6F9" }}>
      <AdminHeader title="Colors Master" />
      ...
      <main style={{ padding: "24px 32px", maxWidth: "1600px", margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: "20px" }}>
          ...
          <div style={{ fontSize: "13px", color: textMuted, marginTop: "4px" }}>
            <span>Home</span> / <span style={{ color: "#0077B6", fontWeight: 600 }}>Colors</span>
          </div>
        </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
              <thead>
                ...
                >
                  ...
                  <th style={{ padding: "12px 16px", width: "160px" }}>Swatch / Icon</th>
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          <button
                            ...
                            onClick={() => {
                              ...
                              setColorHex(item.hexCode || "#0077B6");
                              setErrorMessage("");
            ...
            <form onSubmit={handleCreateSubmit} style={{ padding: "20px" }}>
              ...
              <div style={{ marginBottom: "14px" }}>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Bronze or Rose Gold"
                  value={colorName}
              ...
              <div style={{ marginBottom: "18px" }}>
                ...
                >
                  Icon / Swatch Image URL (Optional)
                </label>
                <input
                  type="text"
      ...
      {showEditModal && editingItem && (
        ...
        >
          ...
          >
            ...
            >
              <h3 style={{ fontSize: "16px", fontWeight: 700, color: textMain, margin: 0 }}>
                Edit Color ({editingItem.code || `CL${editingItem.idNumeric}`})
              </h3>
              <button
                type="button"

...
export default function EditSubcategoryPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      const [subData, cats, attrs] = await Promise.all([
        getAdminSubcategoryBySlug(slug),
        getAdminCategories(),
      ...
      if (subData) {
        ...
        setSubCategoryId(subData.categoryId || (cats[0]?.id ?? ""));
        setSubName(subData.name || "");
        setSubSlug(subData.slug || "");
        setSubTitle(subData.title || "");
        setSubKeywords(subData.keywords || "");
        setSubDescription(subData.description || "");
        setSubPdfCatalogue(subData.pdfCatalogue || "");
        setSubDisplayOrder(subData.displayOrder || 0);
        setSubImage(subData.image || "");
        setSubBanner(subData.banner || "");
        setSubIcon(subData.icon || "");
        setSubStatus(subData.status || "Active");
        setSubIsVisibleWebsite(subData.isVisibleWebsite !== false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!subcategory || !subName.trim() || !subCategoryId) return;

    setSubmitting(true);
    const parentCat = categories.find((c) => c.id === subCategoryId);
    const updatedSlug = subSlug.trim() || subName.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-");

    const updated = await updateAdminSubcategory(subcategory.id, {
    ...
    if (updated) {
      router.push(`/admin/catalogue/subcategories/${updated.slug || updatedSlug}`);
    } else {
  ...
  if (loading) {
    return (
      <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
        <AdminHeader title="Edit Subcategory" theme={theme} onToggleTheme={toggleTheme} />
        <div style={{ padding: "60px", textAlign: "center", color: textMuted }}>Loading subcategory edit form...</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
      <AdminHeader
        ...
        subtitle={`Update attributes, display order, SEO, and R2 media assets for ${subcategory.name}`}
        theme={theme}
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px", background: inputBg, border: `1px solid ${border}`, borderRadius: "12px", padding: "16px" }}>
              ...
              <R2UploadPicker
                ...
                r2Key={`website/catalogue/subcategories/${subcategory.id}/image.webp`}
                currentUrl={subImage}
              ...
              <R2UploadPicker
                ...
                r2Key={`website/catalogue/subcategories/${subcategory.id}/banner.webp`}
              ...
              <R2UploadPicker
                ...
                r2Key={`website/catalogue/subcategories/${subcategory.id}/icon.webp`}
                currentUrl={subIcon}
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: "12px", borderTop: `1px solid ${border}`, paddingTop: "20px", marginTop: "10px" }}>
              ...
              >
                <Save size={16} /> {submitting ? "Updating..." : "Update Subcategory"}
              </button>
            </div>
          </form>
        </div>

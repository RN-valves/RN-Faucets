...
import { AdminCategory, AdminSubcategory, AdminAttributeItem } from "@/types/admin";
...
export default function CreateProductPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([cats, subs, bList, cList, sList, mList]) => {
      setCategories(cats);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const created = await addAdminProduct({
      ...
      image: image || "/api/media/website/catalogue/products/default/image.webp",
    ...
    if (created) {
      router.push(`/admin/catalogue/products/${encodeURIComponent(created.code || code.trim())}`);
    } else {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
      <AdminHeader
        ...
        subtitle="Create a new bathware product item code, rates, specs, and R2 media."
        theme={theme}
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            <div style={{ background: inputBg, border: `1px solid ${border}`, borderRadius: "12px", padding: "16px" }}>
              ...
              <R2UploadPicker
                ...
                r2Key={`website/catalogue/products/new/image.webp`}
                currentUrl={image}
